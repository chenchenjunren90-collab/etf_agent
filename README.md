...
新闻链路采用“规则与事件语言高召回 → DeepSeek 结构化事实抽取 → 原文证据校验 → 历史盈利证据门控”。关键词只负责召回、标签和 LLM 不可用时的降级；语义层必须返回能在标题或正文摘录中逐字核验的证据，并区分已发生/正式宣布、预测/传闻、市场/行业/单家公司影响。单家公司事件不能直接充当 ETF 级盈利证据。组合判断 LLM 默认处于 `audit` 模式，可提供解释和低分空仓提示，但不能直接覆盖量化分数或仓位。
