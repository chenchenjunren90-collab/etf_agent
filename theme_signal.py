...
def get_theme_signals(date_str: str | None = None) -> dict[str, Any]:
    ...
    sentiment = float(signal.get("market_sentiment", 0.0) or 0.0)
    if sentiment >= 0.18:
    ...
    return {
        ...
        "hot_keywords": signal.get("hot_keywords", []),
        "auto_news": {
            "enabled": True,
            "confidence": float(signal.get("confidence", 0.0) or 0.0),
            "market_sentiment": sentiment,
            "article_count": int(signal.get("accepted_count", signal.get("article_count", 0)) or 0),
            "raw_article_count": int(signal.get("article_count", 0) or 0),
            "catalyst_hits": int(signal.get("catalyst_hits", 0) or 0),
            "max_abs_theme": float(signal.get("max_abs_theme", 0.0) or 0.0),
